...
namespace Bones\SemVer\Traits
{
    ...
    trait Incrementable
    {
         ...
         *
         * @return Version|Incrementable This Version object
         */
...
namespace Bones
{
     ...
     */
    define('WPBONES_COMMAND_LINE_VERSION', "1.2.4");

    use Bones\SemVer\Version;
    use Chameleon2die4\WPBonesExtend\Console\Kernel;
    use Exception;
    ...
    class BonesCommandLine
    {
         ...
         * @var Kernel|null
         */
        protected $additionalKernel = null;
  ...
  o   o   o O--o      O--o  o-o o-o  o-o o-o
   \ / \ /  |         |   | | | |  | |-\'  \
    ...
    ';
         ...
         * my-wp-plugin          Internal id used for css, js and fewer files
         *
        ...
        protected function rename($args)
        {
            ...
            if ($ask_continue) {
                $yes_no = $this->ask("Continue (y/n)", 'n');
            ...
            $files = array_merge($files, glob('.env*'));
            ...
            foreach ($files as $file) {
                ...
                // change namespace
                $content = str_replace('Chameleon2die4', $namespace, $content);

                $content = str_replace($search_namespace, $namespace, $content);

                // change slug
                $content = str_replace($this->getPluginSlug($search_plugin_name), $this->getPluginSlug($plugin_name),
                  $content);

                // change vars
                $content = str_replace($this->getPluginVars($search_plugin_name), $this->getPluginVars($plugin_name),
                  $content);

                // change id
                $content = str_replace($this->getPluginId($search_plugin_name), $this->getPluginId($plugin_name),
                  $content);
            ...
            foreach (glob('localization/*') as $file) {
                $newFile = str_replace($this->getPluginId($search_plugin_name), $this->getPluginId($plugin_name),
                  $file);
                rename($file, $newFile);
            }
            ...
            foreach (glob('resources/assets/scripts/*') as $file) {
                $newFile = str_replace($this->getPluginId($search_plugin_name), $this->getPluginId($plugin_name),
                  $file);
                rename($file, $newFile);
            }

            foreach (glob('resources/assets/styles/*') as $file) {
                $newFile = str_replace($this->getPluginId($search_plugin_name), $this->getPluginId($plugin_name),
                  $file);
                rename($file, $newFile);
            }

            foreach (glob('resources/assets/html/*') as $file) {
                $newFile = str_replace($this->getPluginId($search_plugin_name), $this->getPluginId($plugin_name),
                  $file);
                rename($file, $newFile);
            }

            foreach (glob('resources/assets/vue/*') as $file) {
                $newFile = str_replace($this->getPluginId($search_plugin_name), $this->getPluginId($plugin_name),
                  $file);
                rename($file, $newFile);
            }

            foreach (glob('api/*') as $file) {
                $newFile = str_replace($this->getPluginId($search_plugin_name), $this->getPluginId($plugin_name),
                  $file);
                rename($file, $newFile);
            }
         ...
         *     wpdk_rglob_find_file( true, $file ) - when find a file
...
//              }
        ...
        {
            $additionalCommands = $this->additionalKernel->getRealCommands();
            $arguments = $this->arguments();
            $command = $arguments[0];
        ...
        protected function version($argv)
        {
            ...
            $version_number_from_index_php = null;
            $version_number_from_readme_txt = null;
            $version_string_from_index_php = null;
            $stable_tag_version_from_readme_txt = null;
            ...
            $yes_no = $this->ask("The new version of your plugin will be {$version}, is it ok? (y/n)", 'n');
            ...
            if ($version != $version_number_from_index_php || $version != $version_number_from_readme_txt) {
                ...
                $new_stable_tag_version_for_readme_txt = str_replace($version_number_from_readme_txt, $version,
                  $stable_tag_version_from_readme_txt);

                // We're going to change the whole "readme.txt" file
                $new_readme_txt_content = str_replace($stable_tag_version_from_readme_txt,
                  $new_stable_tag_version_for_readme_txt, $readme_txt_content);

                file_put_contents('readme.txt', $new_readme_txt_content);

                // We're going to change the "* Version: x.y.z" with "* Version: $version"
                $new_version_string_for_index_php = str_replace($version_number_from_index_php, $version,
         ...
         */
        protected function createMigrate($className)
        {
            ...
            if (!file_exists($filepath)) {
                ...
                $this->error("File {$filepath} already exist!");
            }
        }
        ...
        protected function createController($className)
        {
            ...
            $filepath = "plugin/Http/Controllers/{$path}{$filename}";
...
//      $command   = str_replace("-", "", $this->sanitize($className));
            ...
            $command = $this->ask("Enter the command:", implode('-', $parts));
            ...
            $filepath = "plugin/Console/Commands/{$filename}";
            if (!file_exists($filepath)) {
                file_put_contents($filepath, $content);
        ...
        protected function createCustomPostType($className)
        {
            ...
            $filepath = "plugin/CustomPostTypes/{$filename}";
            if (!file_exists($filepath)) {
                file_put_contents($filepath, $content);
        ...
        protected function createShortcode($className)
        {
            ...
            $filepath = "plugin/Shortcodes/{$filename}";
            if (!file_exists($filepath)) {
                file_put_contents($filepath, $content);
        ...
        protected function createProvider($className)
        {
            ...
            $filepath = "plugin/Providers/{$filename}";
            if (!file_exists($filepath)) {
                file_put_contents($filepath, $content);
        ...
        protected function createCustomTaxonomyType($className)
        {
            ...
            $filepath = "plugin/CustomTaxonomyTypes/{$filename}";
            if (!file_exists($filepath)) {
                file_put_contents($filepath, $content);
        ...
        protected function createWidget($className)
        {
            ...
            $filepath = "plugin/Widgets/{$filename}";
            if (!file_exists($filepath)) {
                file_put_contents($filepath, $content);
        ...
        protected function createAPIController($className)
        {
            ...
            if (!empty($path)) {
                $content = str_replace('{Path}', $namespacePath, $content);
                mkdir("plugin/Http/Controllers/API/{$path}", 0777, true);
            } else {
                $content = str_replace('{Path}', '', $content);
            }
